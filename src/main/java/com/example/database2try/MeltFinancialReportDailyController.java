...
public class MeltFinancialReportDailyController implements Initializable {
    ...
    private static void getItemsQuantity() throws SQLException, ClassNotFoundException {
        ...
        String SQL = "SELECT items.itemTitle, SUM(orders_items.quantity) as quantity FROM orders_items JOIN orders ON orders_items.orderId = orders.orderId JOIN items ON orders_items.itemId = items.itemId WHERE DAY(orders.order_date) = DAY(NOW()) AND YEAR(orders.order_date) = YEAR(NOW()) AND MONTH(orders.order_date)=MONTH(NOW()) GROUP BY items.itemTitle;";
    ...
    private static void getTotalPrice() throws SQLException, ClassNotFoundException {
        ...
        String SQL = " SELECT SUM(orders.totalPrice) as total_price FROM orders WHERE DAY(orders.order_date) = DAY(NOW()) AND MONTH(orders.order_date) = MONTH(NOW()) AND YEAR(orders.order_date) = YEAR(NOW());";
