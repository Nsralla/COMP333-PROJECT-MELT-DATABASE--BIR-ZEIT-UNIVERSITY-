...
public class MeltFinancialReportYearlyController implements Initializable {
    ...
    private static void getItemsQuantity() throws SQLException, ClassNotFoundException {
        ...
        String SQL = " SELECT items.itemTitle, SUM(orders_items.quantity) as quantity FROM orders_items JOIN orders ON orders_items.orderId = orders.orderId JOIN items ON orders_items.itemId = items.itemId WHERE YEAR(orders.order_date) = YEAR(NOW()) GROUP BY items.itemTitle;";
    ...
    private static void getTotalPrice() throws SQLException, ClassNotFoundException {
        ...
        String SQL = " SELECT SUM(orders.totalPrice) as total_price FROM orders WHERE YEAR(orders.order_date) = YEAR(NOW());";
